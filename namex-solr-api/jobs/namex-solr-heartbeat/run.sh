...
echo "Triggering heartbeat: ${SERVICE_URL}/internal/solr/update/sync/heartbeat"
HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' "${SERVICE_URL}/internal/solr/update/sync/heartbeat")
echo "Response: ${HTTP_CODE}"

if [ "${HTTP_CODE}" != "200" ]; then
  echo "ERROR: Heartbeat returned HTTP ${HTTP_CODE}"
  exit 1
fi
